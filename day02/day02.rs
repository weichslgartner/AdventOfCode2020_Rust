...
fn parse_rule(input: &str) -> PwRule{
    ...
    return PwRule{min_occ: occ_string[0].parse::<usize>().unwrap(),
        max_occ: occ_string[1].parse::<usize>().unwrap(),character: character, pw: password.to_string()};
}

fn part1(rule: &PwRule) -> bool{
    let count = rule.pw.chars().filter(|x| *x==rule.character).count();
    return count >= rule.min_occ && count <= rule.max_occ;
}

fn part2(rule: &PwRule) -> bool{
    let to_match = rule.character as u8;
    return (rule.pw.as_bytes()[rule.min_occ-1]==to_match) ^ (rule.pw.as_bytes()[rule.max_occ-1]==to_match);
}
